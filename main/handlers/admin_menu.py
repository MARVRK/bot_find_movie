...
async def category(message: Message, state: FSMContext):
    ...
    await message.answer("Add code for the movie")
    await state.update_data(category=message.text)
    await state.set_state(Movie.movie_code)


@router.message(Movie.movie_code)
...
async def photo(message: Message, state: FSMContext):
    admin_photo = message.photo[-1].file_id
    file_info = await bot.get_file(admin_photo)
